...
import re
import webbrowser
#from new_interface_qt.seal_client import send_request, save_json
...
from anylabeling.views.labeling.utils import img_arr_to_b64
...
def save_json(data, filename="result.json"):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(data, f)  # indent для читаемого форматирования
...
class LabelingWidget(LabelDialog):
    ...
    def __init__(
        ...
        def test():
            ...
            image_names = [f for f in os.listdir(folder) if os.path.splitext(f)[1].lower() in image_extensions]
            for img_name in image_names:
                result_json = send_request(folder + img_name)
                ...
                for box, label in zip(result_json["boxes"], result_json["labels"]):
                    x_min, y_min, x_max, y_max = box

                    shape = {
                        "label": str(label),
                        "text": "",
                        "points": [[x_min, y_min], [x_max, y_min], [x_max, y_max], [x_min, y_max]],  # Прямоугольник
                        "group_id": None,
                        "shape_type": "polygon",
                        "flags": {}
                    }
                ...
                anylabeling_data["imagePath"] = img_name
                anylabeling_data["imageData"] = None
                anylabeling_data["imageHeight"] = 3024  # Укажите реальную высоту изображения
                anylabeling_data["imageWidth"] = 4032  # Укажите реальную ширину изображения

                save_json(anylabeling_data, folder + img_name.split(".")[0] + ".json")

            self.import_image_folder(folder)
